...
public class StrUtil {
	...
	public static boolean isBlank(CharSequence str) {
		...
		return IntStream.range(0, length).noneMatch(i -> false == CharUtil.isBlankChar(str.charAt(i)));
	}
	...
	public static boolean containsOnly(CharSequence str, char... testChars) {
		if (false == isEmpty(str)) {
			int len = str.length();
			return IntStream.range(0, len).noneMatch(i -> false == ArrayUtil.contains(testChars, str.charAt(i)));
		}
		return true;
	}
	...
	public static String[] wrapAll(CharSequence prefix, CharSequence suffix, CharSequence... strs) {
		final String[] results = Arrays.stream(strs).map(str -> wrap(str, prefix, suffix)).toArray(String[]::new);
		return results;
	}
	...
	public static String[] wrapAllIfMissing(CharSequence prefix, CharSequence suffix, CharSequence... strs) {
		final String[] results = Arrays.stream(strs).map(str -> wrapIfMissing(str, prefix, suffix)).toArray(String[]::new);
		return results;
	}
	...
	public static int count(CharSequence content, char charForSearch) {
		...
		count = (int) IntStream.range(0, contentLength).filter(i -> charForSearch == content.charAt(i)).count();
		return count;
	}
	...
	public static String[] cut(CharSequence str, int partLength) {
		...
		final String str2 = str.toString();
		array = IntStream.range(0, part).mapToObj(i -> str2.substring(i * partLength, (i == part - 1) ? len : (partLength + i * partLength))).toArray(String[]::new);
		return array;
	}
	...
	public static boolean isAllCharMatch(CharSequence value, Matcher<Character> matcher) {
		...
		boolean isAllMatch = IntStream.range(0, len).mapToObj(i -> matcher.match(value.charAt(i))).reduce(true, (a, b) -> a && b);
		return isAllMatch;
	}
	...
	public static int totalLength(CharSequence... strs) {
		int totalLength = Arrays.stream(strs).mapToInt(str -> (null == str ? 0 : str.length())).sum();
		return totalLength;
	}
