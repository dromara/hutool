...
public class CRUDTest {
	...
	public void findLikeTest2() {
		final List<Entity> results = db.findAll(Entity.of("user").set("name", new Condition("name", "三", Condition.LikeType.Contains)));
		Assertions.assertEquals(2, results.size());
	}

	@Test
	public void findLikeTest3() {
		final List<Entity> results = db.findAll(Entity.of("user").set("name", new Condition("name", null, Condition.LikeType.Contains)));
		Assertions.assertEquals(0, results.size());
	}
